...
- After the main agent replies with interface signatures through a follow-up, continue the same proof in the same session. A true blocker is limited to a false/missing hypothesis, circular source argument, or persistently unavailable REPL. A failure whose message starts with REPL-DOWN or REPL-CONFLICT needs the main agent (recovery failed, or another session's REPL generation must be freed): stop retrying (each retry fails fast), note it in your report as an infrastructure blocker with the error text, and continue with any code you have already verified.
