...
/-- GT `ga08`: orbit cardinality equals the cardinality of the stabilizer
quotient, without collapsing infinite cardinals to zero. -/
...
* `e6` is the displayed equation inside `it21`; it is represented by
  `CommGroup.exists_mulEquiv_free_prod_primePower`, and the canonical rank's
  isomorphism invariance is checked by `CommGroup.freeRank_eq_of_mulEquiv`.
  Identifying the displayed number of free factors with that canonical rank,
  and the invariant-factor and elementary-divisor uniqueness clauses, still
  require uniqueness theory not currently present in Mathlib's finite-abelian
  API. Likewise `e34` is the
  displayed orbit--stabilizer equation represented without a finiteness
  collapse by `MulAction.orbit_cardinal_eq_quotient`.
...
* The order-`2p` classification refinement `ga13m` remains beyond the direct
...
* The remaining representation results `r10c`, `r17`, `r23`, `r28`, `r29`,
