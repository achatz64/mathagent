...
/- AUDIT-GAP `it21` / `e6`: require nontrivial torsion factors in the general
existence theorem and expose invariant-factor and elementary-divisor
uniqueness, not only free-rank invariance. -/
...
/- AUDIT-GAP `ga22`: the parity formula is still needed for every supplied
transposition factorization, not only the factorization constructed above. -/
...
with centralizer indices, not only the equivalent conjugacy-class sizes. -/
...
/- AUDIT-GAP `r15`: under the chapter's finite-dimensional `F`-algebra
hypothesis, expose that the resulting division algebra is finite-dimensional
over `F`. Mathlib provides `exists_algEquiv_matrix_divisionRing_finite`. -/
...
/- AUDIT-GAP `r28a`: add the chapter-facing finite-dimensional form, exposing
finite-dimensionality of every division-algebra factor. Mathlib provides
...
basis conclusion still depends on the missing complete enumeration `r32(a)`. -/
