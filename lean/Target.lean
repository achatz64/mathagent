...
/-- GT `bd4`. Supporting definition: a nonempty subset closed under multiplication
and inversion is a subgroup. The construction unpacks the subgroup structure:
the identity lies in `S` because `a⁻¹ * a ∈ S` for any `a ∈ S`. -/
...
/-- GT `bd5`. Technical lemma: membership in an arbitrary intersection of
subgroups. The proof is trivial: it unpacks and repacks definitions via `simp`. -/
theorem Subgroup.mem_sInf_iff {S : Set (Subgroup G)} {x : G} :
    x ∈ sInf S ↔ ∀ H ∈ S, x ∈ H := by
  simp

/-- GT `bd7`, leastness clause: the generated subgroup is the least subgroup
containing the set. The proof is a routine delegation to `Subgroup.closure_le`. -/
...
inverses. The empty list represents `1`; repetitions are allowed. Technical
lemma. Proof idea: forward, induction on closure membership
(`Subgroup.closure_induction`) assembles the word for `x` from the words of the
factors, concatenating lists (reversing and inverting for an inverse step);
backward, the product of a list of elements of `S` and their inverses lies in
the closure by `Subgroup.list_prod_mem`. -/
...
permutation group. Key construction: the left-regular action embeds `G` in the
permutation group of its underlying set. The proof is a routine delegation to
...
`n` letters. Technical lemma. Proof idea: transport the left-regular
permutation representation along a bijection `G ≃ Fin n` given by the
cardinality hypothesis (`Fintype.equivOfCardEq` and `Equiv.Perm.permCongrHom`),
and compose with `MulAction.toPermHom`; injectivity follows from the action
being faithful. -/
...
/-- GT `bd14(a)`: a point of a left coset determines that coset. Technical
lemma. Proof idea: membership `a ∈ b • H` says `b⁻¹ * a ∈ H`, which by
`leftCoset_eq_iff` is exactly the equality of the two cosets; the converse
witnesses `a` as `a * 1`. -/
...
/-- GT `bd14(b)`: two left cosets are equal or disjoint. Technical lemma.
Proof idea: if a point `x` lies in both cosets, applying
`Subgroup.mem_leftCoset_iff_eq_leftCoset` to `x` in each coset shows both equal
`x • H`, hence equal to each other. -/
...
/-- GT `bd14`, part (c): equality of left cosets. Technical lemma. The proof is
a routine delegation to `leftCoset_eq_iff`. -/
...
Supporting construction: multiplication by `a` restricts to a bijection
`a • H ≃ H`. The proof is a routine delegation to `H.leftCosetEquivSubgroup`. -/
def Subgroup.leftCosetEquiv (H : Subgroup G) (a : G) : (a • H : Set G) ≃ H :=
  H.leftCosetEquivSubgroup a

/-- GT `bd15` (Lagrange), in `Nat.card` form. Technical lemma: the order of a finite
group is the product of the order of a subgroup and its index. The proof is a
routine delegation to `H.card_mul_index`. -/
...
order of the ambient group. Technical lemma. The proof is a routine delegation
to `H.card_subgroup_dvd_card`. -/
...
Technical lemma. The proof is a routine delegation to `orderOf_dvd_natCard`
applied to the cyclic subgroup generated by the element. -/
theorem orderOf_dvd_group_card (x : G) : orderOf x ∣ Nat.card G :=
  orderOf_dvd_natCard x

/-- GT `bd19`: multiplicativity of subgroup indices in a tower. Technical
lemma. The proof is a routine delegation to `Subgroup.relIndex_mul_index`. -/
...
Technical lemma. The proof is a routine delegation to `normal_iff_eq_cosets`. -/
...
/-- GT `bd25`: if `N` is normal, the carrier of `H ⊔ N` is the pointwise
product `HN`. Technical lemma. The proof is a routine delegation to
`Subgroup.mul_normal`. -/
...
Technical lemma. The proof is trivial: it is found by instance search
(`infer_instance`). -/
theorem Subgroup.sup_normal (H N : Subgroup G) [H.Normal] [N.Normal] :
    (H ⊔ N).Normal := by
  infer_instance

/-- GT `bd25l`: a conjugation-stable set generates a normal subgroup. Key
lemma. Proof idea: it suffices that every `g` normalizes the closure; conjugation
by `g` maps generators of `S` into `S`, so the conjugated closure equals the
closure itself (`MonoidHom.map_closure` plus a pointwise equality on the
underlying set). -/
...
least conjugation-stable set containing `S`. Technical lemma. Proof idea: an
element of the conjugates-of-set is a conjugate of some element of `S`, and
conjugating it again is still a conjugate of an element of `S`
(`Group.conj_mem_conjugatesOfSet`, `Group.mem_conjugatesOfSet_iff`); leastness
follows by pushing the conjugation into `T`. -/
...
by all conjugates of elements of `S`. Technical lemma. The proof is trivial:
the two subgroups are definitionally equal (`rfl`). -/
...
/-- GT `bd26`: kernels of group homomorphisms are normal. Technical lemma.
The proof is trivial: it is found by instance search (`infer_instance`). -/
theorem MonoidHom.ker_normal (f : G →* G') : f.ker.Normal := by
  infer_instance

/-- GT `bd27`: a normal subgroup is the kernel of its quotient map. Technical
lemma. The proof is a routine delegation to `QuotientGroup.ker_mk'`. -/
...
forced by the projection agree with Mathlib's quotient operations. Technical
lemma. Proof idea: every quotient element has a representative
(`QuotientGroup.mk'_surjective`), so any operation agreeing with Mathlib's on
representatives agrees everywhere, by `funext` plus the hypotheses. -/
...
killing `N` factors through it, and the factor is unique. Key lemma of the
section. Proof idea: the factor is `QuotientGroup.liftOfLeKer N f h`; existence
holds pointwise since every coset has a representative, and uniqueness follows
by cancelling the surjective quotient map
(`MonoidHom.cancel_right` with `QuotientGroup.mk'_surjective`). -/
...
Technical lemma. The proof is trivial: it is a pointwise computation, hence
`ext x; rfl`. -/
...
Technical lemma. The proof is a routine delegation to
`Subgroup.relIndex_map_map`, with the suprema simplifying to the subgroups
because the kernels are contained in them (`sup_eq_left`). -/
...
normal exactly when its image is normal. Technical lemma. Proof idea:
forward, `H = (H.map f).comap f` since `f.ker ≤ H`, and preimages of normal
subgroups are normal; backward, normality passes to images along surjections
(`Subgroup.Normal.map`). -/
...
a corresponding normal subgroup. Auxiliary construction. Proof idea: apply the
induced quotient map `QuotientGroup.map H (H.map f) f` and check bijectivity:
injectivity from `QuotientGroup.ker_map` and `Subgroup.comap_map_eq_self`,
surjectivity from `QuotientGroup.map_surjective_of_surjective`. -/
...
Technical lemma. The proof is a routine delegation to
`Subgroup.normal_map_iff_of_surjective` for the surjective quotient map, whose
kernel is `N` by `QuotientGroup.ker_mk'`. -/
...
/-- Supporting definition of the section: the paper's notion that
multiplication identifies `H × K` with `G` as a group, with the specified
multiplication map rather than an arbitrary isomorphism. -/
def Subgroup.IsInternalDirectProduct (H K : Subgroup G) : Prop :=
  ∃ e : (H × K) ≃* G, ∀ x, e x = (x.1 : G) * (x.2 : G)

/-- Technical lemma: multiplication is an internal direct-product isomorphism
exactly when it is bijective and the two subgroups commute elementwise.
Proof idea: forward, bijectivity of the multiplication map is the definition of
`IsComplement'`, and commutation is extracted by comparing
`e (h, 1) * e (1, k)` computed two ways via the pair multiplication law;
backward, commutation makes the pointwise multiplication a homomorphism, and
`MulEquiv.ofBijective` turns the complement' bijectivity into an equivalence. -/
...
Key lemma of the section. Proof idea: combine the preceding complement'
characterization with the translation between complement' and
`sup_eq_top`/`Disjoint`; injectivity uses the standard trick that a common
refinement of a product decomposition of `1` is trivial, surjectivity factors
each `g ∈ H ⊔ K` through `Subgroup.coe_mul_of_left_le_normalizer_right` using
commutation to see `H` normalizes `K`. -/
...
replaced by normality of both factors. Technical lemma. Proof idea: normality
of both factors with trivial intersection gives commutation via
`Subgroup.commute_of_normal_of_disjoint`; conversely, commutation shows each
factor normalizes the other and itself, so both normalizers are all of `G` by
...
/-- Supporting definition: the ordered pointwise product of a list of
subgroups, i.e. the set of products taken in the given order. -/
def Subgroup.orderedProduct {ι : Type*} (H : ι → Subgroup G)
    (l : List ι) : Set G :=
  (l.map fun i => (H i : Set G)).prod

/-- Supporting definition: the ordered multiplication map on a `Fin`-indexed
family of subgroups, evaluating the product of the coordinate values in the
canonical `Fin` order. -/
def Subgroup.piMul {n : ℕ} (H : Fin n → Subgroup G)
    (x : (i : Fin n) → H i) : G :=
  (List.finRange n |>.map fun i => (x i : G)).prod

/-- Key definition of the section: multiplication identifies the external
product of a finite ordered family of subgroups with the ambient group, with
the displayed ordered multiplication map rather than an arbitrary isomorphism. -/
...
/-- GT `it07`. Key lemma of the section: an ordered finite family is an
internal direct product exactly when its ordered pointwise product is all of
`G`, each factor meets the ordered product of all the other factors only in
`1`, and every factor is normal. Proof idea: forward, surjectivity of `e`
gives fullness, injectivity compares two tuples differing only at `j` (using
the single-coordinate and erase-one list-product identities for nodup lists)
to force the intersection clause, and normality follows by conjugating a tuple
supported at `j`; backward, normality plus trivial pairwise intersections give
commutation (`Subgroup.commute_of_normal_of_disjoint`), which makes the ordered
multiplication a homomorphism, surjective by fullness and injective by the
intersection clause. -/
...
/-- GT `it15`. Technical lemma: an action gives the displayed group law on
pairs; the canonical copies of both factors form an internal semidirect product
with that action. Proof idea: everything unfolds from the pair group law;
normality of the first factor comes from
`SemidirectProduct.range_inl_eq_ker_rightHom`, and the intersection, product,
and conjugation clauses are computed from representatives of the two factors. -/
...
/-- GT `st14`. Auxiliary construction: conjugating an action on `N` gives an
isomorphic semidirect product via `(n,q) ↦ (α(n),q)`. The proof is a routine
delegation to `SemidirectProduct.congr`, checking the action condition
pointwise on the automorphism `α`. -/
...
/-- GT `st15`. Auxiliary construction: precomposing an action by an
automorphism of `Q` gives an isomorphic semidirect product via
`(n,q) ↦ (n,α(q))`. The proof is a routine delegation to
`SemidirectProduct.congr`, the action condition being definitional after
rewriting with the hypothesis on `θ`. -/
...
/-- GT `st16`. Key lemma of the section: semidirect products by a finite
cyclic group are isomorphic when the ranges of their actions are conjugate in
`MulAut N`. Proof idea: replace `θ` by its conjugate `θc` using
`SemidirectProduct.conjugateActionMulEquiv`; since `Q` is cyclic, two actions
with equal range differ by precomposition with an automorphism of `Q` (the
private lemma `cyclic_hom_eq_comp_mulAut_of_range_eq`, which transports the
generator along the induced automorphism of `MulAut N`), and
`SemidirectProduct.precompActionMulEquiv` finishes the chain. -/
...
/-- GT `it19`, finite-family span-preserving form. Technical lemma: if the
gcd of the coefficients is one, the family can be replaced by a spanning family
with a prescribed first vector. The proof is a routine delegation to the private
lemma `exists_family_span_eq_with_eq_zsum`, converting the `ℤ`-scalar
conclusion into the `ℕ`-scalar form. -/
...
the commutativity hypothesis used in the text. Technical lemma. The proof is a
routine delegation to `isCyclic_of_card_pow_eq_one_le`. -/
...
prime-power (elementary-divisor) form. The proof is a routine delegation to
...
/-- Technical lemma: the gcd of two prime powers records the smaller exponent
when the primes agree, and is one otherwise. Proof idea: split on `q = p`; if
equal, the gcd is the smaller power by the `gcd`-cancellation lemmas for
divisibility, otherwise the powers are coprime by `Nat.coprime_primes`. -/
...
/-- Technical lemma: the gcd with a prime power is controlled by the
corresponding prime valuation. Proof idea: compare prime factorizations via
`Nat.eq_of_factorization_eq`; at each prime `q` the valuation of the gcd is the
minimum of the valuations (`Nat.factorization_gcd`, `Nat.Prime.factorization_pow`),
which reads off as `min (n.factorization p) k`. -/
...
/-- Auxiliary construction: taking `d`-th roots of one commutes with a finite
product of commutative groups, componentwise. The proof is trivial: it unpacks
and repacks the subtype and product structures pointwise. -/
...
/-- Auxiliary construction: a multiplicative equivalence restricts to an
equivalence between its subtypes of `d`-th roots of one. Proof idea: apply the
equivalence and its inverse, using `map_pow` to transfer the root condition. -/
...
/-- Technical lemma: the number of `d`-th roots of one in a finite product of
finite cyclic groups is the product of the corresponding gcds. Proof idea:
transport along `CommGroup.powEqOnePiEquiv`, use `Nat.card_pi`, and evaluate
each factor by `IsCyclic.card_powMonoidHom_ker`. -/
...
/-- Technical lemma: the root-count profile of a product of cyclic
prime-power groups. Proof idea: reduce to the cyclic gcd formula via
`CommGroup.card_pow_eq_one_pi_cyclic`, evaluate each gcd with
`Nat.gcd_prime_pow_prime_pow` (so only the factors with `q i = p` contribute),
and collect the surviving exponents with `Finset.prod_ite` and
...
/-- Technical lemma: the root-count profile of a product of arbitrary
nontrivial cyclic groups, expressed through prime valuations. Proof idea: the
same reduction via `CommGroup.card_pow_eq_one_pi_cyclic`, with each gcd
evaluated by `Nat.gcd_prime_pow_eq_pow_min_factorization` and the product
converted to a power of a sum. -/
...
/-- Technical lemma: isomorphic products of nontrivial cyclic groups have
identical truncated prime-valuation profiles. Proof idea: transport the count
of `(p^k)`-th roots of one through `CommGroup.powEqOneEquiv`, rewrite both
sides with `CommGroup.card_pow_eq_one_pi_cyclic_factorization`, and conclude by
injectivity of `x ↦ p ^ x` for prime `p`. -/
...
/-- Technical lemma: successive differences of the sum of truncated natural
numbers count how many entries lie above the truncation point. Proof idea:
split the extra summand as a sum of indicators
(`Finset.card_eq_sum_ones`, `Finset.sum_filter`), then compare `min (a i) k`
and `min (a i) (k+1)` by cases on `k < a i` (closed by `omega`). -/
...
/-- Technical lemma: partition the entries strictly above `k - 1` into those
equal to `k` and those strictly above `k`. Proof idea: the two filtered sets
are disjoint and their union is the filter on the left, so the cardinalities
add by `Finset.card_union_of_disjoint`; membership arithmetic is by `omega`. -/
...
/-- Technical lemma: a nondecreasing finite sequence is determined by the
cardinalities of all of its strict upper level sets. Proof idea: suppose
`a i > b i` at some index; then the level set of `a` above `b i` contains the
up-set `{j ≥ i}`, while the corresponding level set of `b` is contained in
`{j > i}`, whose cardinalities differ by one — contradicting the assumed
equality of upper level cardinalities (via `Fin.card_Ici`/`Fin.card_Ioi` and
`omega`); the reverse inequality is symmetric. -/
...
/-- Technical lemma: isomorphic products of nontrivial cyclic prime-power
groups have the same root-count exponent profile at every prime and exponent.
Proof idea: transport the count of `(r^k)`-th roots of one through
`CommGroup.powEqOneEquiv`, rewrite with
`CommGroup.card_pow_eq_one_pi_prime_power` on both sides, and conclude by
injectivity of `x ↦ r ^ x` for prime `r`. -/
...
/-- Technical lemma: the free abelian group of rank `r` needs exactly `r`
generators. The proof is a routine delegation to
`Group.rank_multiplicative_eq_finrank` together with
...
commutative-group isomorphism. Technical lemma. The proof is a routine
delegation to `CommGroup.freeRank_congr`. -/
...
/-- Technical lemma: adjacent divisibility in a finite sequence implies
divisibility of every earlier term into every later term. Proof idea: induction
on the second index with `Fin.induction`, chaining the adjacent divisibilities
by `Nat.dvd_trans` (with the endpoint cases settled by reflexivity and the
index bookkeeping by `omega`). -/
...
/-- Auxiliary construction: quotienting a product of a torsion-free group and
a torsion group by its torsion subgroup recovers the first factor. Proof idea:
the torsion subgroup of `A × B` is exactly the kernel of the first projection
(`IsOfFinOrder.prod_iff` and torsion-freeness of `A`), so the quotient is
`A` by `QuotientGroup.quotientMulEquivOfEq` and
...
/-- Technical lemma: a displayed free factor in a finitely generated
commutative-group decomposition has the intrinsic free rank. The proof is a
routine delegation to `CommGroup.freeFactorMulEquiv` (identifying `G ⧸ torsion G`
with the free factor) and `Group.rank_pi_multiplicative_int`. -/
...
/-- GT `it21`, full-decomposition torsion uniqueness. Key lemma of the
section: when two decompositions of the same finitely generated commutative
group include their free factors, the invariant factors of their finite torsion
factors agree. Proof idea: transport the torsion part of each decomposition to
an isomorphism between the two torsion factors via
`CommGroup.torsionFactorMulEquiv`, reducing torsion uniqueness to
`CommGroup.invariantFactors_unique`; the free ranks agree by
`CommGroup.freeRank_eq_of_free_prod_torsion`. -/
...
Key lemma of the section: two prime-power decompositions have the same free
rank and the same multiplicity for every elementary divisor `q^k`. Proof idea:
as in the invariant-factor case, the torsion factors are isomorphic via
`CommGroup.torsionFactorMulEquiv`, and
`CommGroup.card_prime_power_factors_eq_of_mulEquiv` compares multiplicities of
each elementary divisor. -/
...
/-- GT `it22`, part (b). Key construction of the section: the evaluation map
identifies a finite commutative group with its double dual. The proof is a
routine delegation to `(CommGroup.monoidHomMonoidHomEquiv G M).symm`. -/
...
/-- GT `it24`, orthogonality. Key lemma of the section: over a field, two
unit-valued characters of a finite commutative group have scalar product `|G|`
when equal and zero otherwise. Proof idea: each summand is the value of the
character `chi * psi⁻¹` composed into `k`, and this character is trivial
exactly when `chi = psi` (comparing values via `Units.ne_zero` and
`mul_inv_eq_one₀`); the sum of a nontrivial character vanishes by
`sum_hom_units`, while the trivial one sums to `|G|`. -/
...
open scoped Classical in
/-- GT `it25`. Technical lemma: summing all characters at a fixed group
element gives `|G|` at the identity and zero elsewhere. Proof idea: the finsum
is a finite sum since its support is contained in the universal finset
(`finsum_eq_sum_of_support_subset`); it is the orthogonality relation for the
evaluation character against the trivial character
(`CommGroup.sum_character_mul_inv_character`), with the dual group counted via
the duality equivalence of `CommGroup.monoidHom_mulEquiv_of_hasEnoughRootsOfUnity`. -/
...
/-- GT `fg03`: uniqueness in the universal property of the free group. Technical
lemma: two homomorphisms out of a free group agreeing on the generators are
equal. Proof idea: free-group extensionality tests equality of homomorphisms
on generators, reducing the claim to the hypothesis. -/
...
group is surjective. The proof is trivial: the preimage of any `g` is the
generator `FreeGroup.of g`, via `FreeGroup.lift_apply_of`. -/
...
/-- GT `fg06` (Nielsen--Schreier): subgroups of free groups are free. Key
theorem of the section. The proof is trivial: it is the registered instance
that a subgroup of a free group is free, found by `infer_instance`. -/
...
Technical lemma: two homomorphisms out of a presented group agreeing on the
generators are equal. Proof idea: presented-group extensionality tests
equality on generators, reducing the claim to the hypothesis. -/
...
stabilizer quotient to the set is `G`-equivariant. Key construction of the
section. Proof idea: the orbit map `G ⧸ stabilizer G x → X` is bijective
(injectivity by `MulAction.injective_ofQuotientStabilizer`, surjectivity by
transitivity), so `Equiv.ofBijective` packages it, and equivariance is
...
stabilizer quotient, without collapsing infinite cardinals to zero. Technical
lemma. The proof is a routine delegation to `Cardinal.mk_congr`, applied to a
composition of universe-lifting equivalences with
...
normalizer, valid without finiteness assumptions. Auxiliary construction.
Proof idea: transport `MulAction.orbitEquivQuotientStabilizer` for the
conjugation action along the group isomorphism `ConjAct G ≃ G`, which carries
the stabilizer of `H` to the normalizer; the two quotient maps are built with
`Quotient.map'`. -/
...
coset quotient by the normalizer, with no finiteness assumption. Technical
lemma. The proof is trivial: it is `Cardinal.mk_congr` applied to
`Subgroup.conjugationOrbitEquivNormalizerQuotient`. -/
...
/-- GT `ga10`: the kernel of the coset action is the normal core. Technical
lemma. The proof is a routine delegation to `H.normalCore_eq_ker`. -/
...
largest normal subgroup contained in a chosen point stabilizer. Key lemma of
the section. Proof idea: the kernel fixes every point, hence fixes `x`;
conversely, if `N` is normal and fixes `x`, then `g⁻¹ * n * g` also fixes `x`,
so `n` fixes `g • x` — every normal subgroup in a point stabilizer lies in
the kernel. -/
...
of the stabilizer indices of orbit representatives. Key lemma. Proof idea:
rewrite the cardinality along the decomposition of `X` as a sigma type over
its orbits, apply `Fintype.card_sigma`, and evaluate each summand with
...
to maximality of a point stabilizer. Technical lemma. The proof is a routine
delegation to `MulAction.isCoatom_stabilizer_iff_preprimitive`. -/
...
/-- GT `e18`: the block condition is exactly that every translate is either
equal to the block or disjoint from it. Technical lemma. The proof is a
routine delegation to the identical Mathlib result of the same name
(`_root_.MulAction.isBlock_iff_smul_eq_or_disjoint`). -/
...
has a proper nonsingleton block. Technical lemma. Proof idea: preimitivity
says every block is trivial; in the forward direction a nontrivial block is
extracted from the failure of that triviality requirement, and conversely
`IsPreprimitive.isTrivialBlock_of_isBlock` rules out a proper nonsingleton
block. -/
...
/-- Inclusion helper for GT `ga38`. Technical lemma: the stabilizer of a point
of a block is contained in the stabilizer of the block. The proof is a
routine delegation to `hB.stabilizer_le`. -/
...
strictly contained in the whole group. Key lemma of the section. Proof idea:
pick `y ≠ x` in `B` and `z ∉ B`; an element carrying `x` to `y` lies in the
block stabilizer but not the point stabilizer, and an element carrying `x` to
`z` lies outside the block stabilizer — transitivity supplies both. -/
...
cycle factors. Technical lemma. The proof is a routine delegation to
...
the parity of the displayed factorization length. Technical lemma. Proof
idea: take Mathlib's chosen swap factorization `Equiv.Perm.swapFactors f` and
compute the sign of the product of swaps with `Equiv.Perm.sign_prod_list_swap`. -/
...
factorization, not only Mathlib's chosen factorization. Technical lemma. The
proof is a routine delegation to `Equiv.Perm.sign_prod_list_swap`. -/
...
/-- GT `ga23`: the alternating group is generated by three-cycles. Key
theorem of the section. The proof is a routine delegation to
...
types agree. Key lemma. The proof is a routine delegation to
...
/-- GT `ga12` / `e36`: conjugacy classes partition a finite group. Key lemma.
The name is retained (not snake_case) because the body self-references this
declaration under its original name. The proof is a routine delegation to
...
noncentral conjugacy classes. Key lemma. The proof is a routine delegation to
...
/-- A chosen representative of a conjugacy class. Auxiliary construction:
defined as the quotient's canonical choice `Quotient.out C`. -/
...
representative. Technical lemma. Proof idea: identify the centralizer of the
representative with the stabilizer of its class under the conjugation action
(comap along `ConjAct.toConjAct`), and use the orbit--stabilizer index
formula together with the identification of the orbit with the class carrier. -/
...
of chosen conjugacy-class representatives. Key lemma. Proof idea: rewrite
each centralizer index as the size of the corresponding conjugacy class via
`ConjClasses.centralizerIndex_representative`, then apply the class equation
`Group.sum_card_conj_classes_eq_card`. -/
...
Key lemma. Proof idea: rewrite each centralizer index as the class size via
`ConjClasses.centralizerIndex_representative`, then apply
`Group.nat_card_center_add_sum_card_noncenter_eq_card`. -/
...
/-- GT `ga13` (Cauchy): a prime divisor of the group order occurs as an
element order. Key lemma. The proof is a routine delegation to
...
Technical lemma. The proof is a routine delegation to `IsPGroup.iff_orderOf`. -/
...
/-- GT `ga14`: a nontrivial finite `p`-group has nontrivial centre. Key
lemma. The proof is a routine delegation to the identical Mathlib result of
the same name (`hG.center_nontrivial`). -/
...
subgroup of order `p^m` for every `m ≤ n`. Key lemma. Proof idea: induction
on `m` — given a proper normal `N` of order `p^m`, the quotient `G ⧸ N` is a
nontrivial `p`-group with nontrivial centre, so it contains a central element
of order `p`; the preimage of its cyclic subgroup under the quotient map is
normal of order `p ^ (m + 1)`. -/
...
/-- GT `ga17`: a cyclic quotient by the centre forces commutativity. Key
lemma. The proof is a routine delegation to
...
subgroup, rather than necessarily by the full centre, suffices. Technical
lemma. The proof is a routine delegation to
`QuotientGroup.mk'.isMulCommutative_of_isCyclic_of_ker_le_center`, after
checking that the kernel of the quotient map lies in the centre. -/
...
/-- GT `ga16`: a group of prime-square order is commutative. Key lemma. The
proof is a routine delegation to `IsPGroup.isMulCommutative_of_card_eq_prime_sq`. -/
...
cyclic of order `p²` or elementary abelian of rank two. Key theorem of the
section. Proof idea: if `G` is cyclic, transport the `ZMod p²` identification
with `zmodCyclicMulEquiv`; otherwise the exponent is `p`, which makes the
additive group a free `ZMod p`-module of rank `2` (cardinality comparison via
`Module.natCard_eq_pow_finrank`), hence `G ≅ ZMod p × ZMod p`. -/
...
/-- GT `ga28`: alternating groups on at least five letters are simple. Key
theorem of the section. The proof is a routine delegation to
...
group is trivial, alternating, or the whole symmetric group. Key theorem of
the section. Proof idea: any nontrivial normal `N` contains the alternating
group (`Equiv.Perm.alternatingGroup_le_of_normal`), so the index of `N`
divides `2` (`Subgroup.index_dvd_of_le`); index `2` identifies `N` with the
alternating group (`Equiv.Perm.eq_alternatingGroup_of_index_eq_two`),
otherwise `N = ⊤`. -/
...
resolved as a false positive (rename would break the field reference). The
proof is a routine delegation to Mathlib's field projection
`hG.card_modEq_card_fixedPoints`. -/
...
/-- GT `st2`: Sylow I in its prime-power divisor form. Key lemma. The proof
is a routine delegation to `Sylow.exists_subgroup_card_pow_prime`. -/
...
/-- GT `st7`: all Sylow subgroups are conjugate. Key lemma. The proof is a
routine delegation to `MulAction.exists_smul_eq` (transitivity of the
...
/-- GT `st7(b)`: the number of Sylow subgroups is one modulo `p`. Key lemma.
The proof is a routine delegation to `card_sylow_modEq_one`. -/
...
Technical lemma. The proof is a routine delegation to `P.card_dvd_index`. -/
theorem Sylow.card_dvd_index' [Fact p.Prime] [Finite (Sylow p G)] (P : Sylow p G) :
    Nat.card (Sylow p G) ∣ P.index :=
  P.card_dvd_index

/-- GT `st7(c)`: every `p`-subgroup lies in a Sylow `p`-subgroup. Key lemma.
The proof is a routine delegation to `hH.exists_le_sylow`. -/
...
contained in it. Technical lemma. Proof idea: `H ⊓ normalizer P` is a
`p`-subgroup normalizing `P`, and `hH.inf_normalizer_sylow` pins it inside
`P`, so `H ≤ P`. -/
...
normalize `P`. Technical lemma. Proof idea: `Q` is a `p`-subgroup normalizing
`P`, hence lies in `P` by `IsPGroup.le_sylow_of_le_normalizer`; maximality of
the Sylow subgroup `Q` (`Q.is_maximal'`) then forces `Q = P`. -/
...
Auxiliary construction. Proof idea: fix any Sylow subgroup `R` of `H`; by
`Sylow.exists_comap_subtype_eq` it is the comap of some Sylow `Q` of `G`, and
`Q` is conjugate to `P` by transitivity of the conjugation action. -/
...
is self-normalizing. Key lemma. Proof idea: the containment `⊇` is the
normalizer property; for `⊆`, take `g` normalizing `H` — then `g • P` is a
Sylow subgroup of `G` lying in `H`, hence conjugate inside `H` to `P` by some
`h ∈ H`, and `h * g` normalizes `P`, so `g ∈ H`. -/
...
`Sylow p G` is already inhabited. Technical lemma. The proof is a routine
delegation: normality yields uniqueness via `Sylow.unique_of_normal`, and
uniqueness yields normality via `Sylow.normal_of_subsingleton`. -/
...
the book's finite product. Key lemma. Proof idea: for each prime `p` dividing
the group order, the uniqueness hypothesis makes the Sylow subgroup normal
(`Sylow.normal_of_subsingleton`); this is exactly the hypothesis of the
direct-product clause of Mathlib's finite-nilpotency criterion
(`Group.isNilpotent_of_finite_tfae`, implication 3 → 4), which supplies the
...
/-- GT `ns21` (Frattini's argument). Key lemma. The proof is a routine
delegation to `P.normalizer_sup_eq_top`. -/
...
/-- GT `it17` (Schur--Zassenhaus), right-complement form. Key theorem of the
section. The proof is a routine delegation to
`N.exists_left_complement'_of_coprime`. -/
...
/-- The exact statement cited as GT `ns04` (Feit--Thompson). Supporting
definition: the `Prop`-valued statement of the Feit--Thompson theorem,
packaged so that the axiom recording the external dependency can cite it. -/
...
/-- GT `ns06(a)`: subgroups of solvable groups are solvable. Technical lemma.
The proof is trivial: it is the registered instance for subgroups of solvable
groups, found by `infer_instance`. -/
theorem Subgroup.isSolvable [IsSolvable G] (H : Subgroup G) :
    IsSolvable H := by
  infer_instance

/-- GT `ns06(a)`: quotients of solvable groups are solvable. Technical lemma.
The proof is trivial: it is the registered instance for quotients of solvable
groups, found by `infer_instance`. -/
...
extensions. Key lemma. The proof is a routine delegation to
...
/-- GT `ns07`: finite `p`-groups are solvable. Technical lemma. The proof is
a routine delegation to `IsPGroup.isNilpotent` (finite `p`-groups are
...
/-- GT `ns09`: the commutator subgroup is characteristic. Technical lemma.
The proof is trivial: it is the registered instance for the commutator
subgroup, found by `infer_instance`. -/
...
commutative quotient. Technical lemma. The proof is a routine delegation to
...
/-- GT `ns10`: derived-series characterization of solvability. Technical
lemma. The proof is a routine delegation to `isSolvable_def`. -/
theorem Group.isSolvable_iff_derivedSeries_eq_bot :
    IsSolvable G ↔ ∃ n : ℕ, derivedSeries G n = ⊥ :=
  isSolvable_def G

/-- GT `ns12(a)`: subgroups of nilpotent groups are nilpotent. Technical
lemma. The proof is trivial: it is the registered instance for subgroups of
nilpotent groups, found by `infer_instance`. -/
theorem Subgroup.isNilpotent [Group.IsNilpotent G] (H : Subgroup G) :
    Group.IsNilpotent H := by
  infer_instance

/-- GT `ns12(b)`: quotients of nilpotent groups are nilpotent. Technical
lemma. The proof is trivial: it is the registered instance for quotients of
nilpotent groups, found by `infer_instance`. -/
...
/-- The left-normed commutator `[...[g 0, g 1], ... , g m]`. Auxiliary
construction: defined by recursion on a list via the private helper
`leftNormedCommutatorList`. -/
...
left-normed commutator of `m + 1` elements is trivial. Key lemma of the
section. Proof idea: nilpotency class at most `m` is equivalent to the `m`-th
upper central series being everything; membership in the upper central series
unfolds, via the private list-level criterion
`mem_upperCentralSeries_iff_leftNormedCommutatorList_eq_one`, to the
vanishing of all left-normed commutators of length `m + 1`. -/
...
/-- GT `ns16`: finite `p`-groups are nilpotent. Technical lemma. The proof is
a routine delegation to `hG.isNilpotent`. -/
...
Technical lemma. The proof is a routine delegation to Mathlib's criterion
`Group.isNilpotent_of_finite_tfae`, reused verbatim. -/
...
Key theorem of the section. The proof is trivial: it is the registered
instance for group algebras with `NeZero (Nat.card G)`, found by
`infer_instance`. -/
theorem group_algebra_isSemisimple
    [NeZero (Nat.card G : k)] : IsSemisimpleRing (MonoidAlgebra k G) := by
  infer_instance

/-- GT `r3` (Maschke), in the paper's invariant-complement form. Key lemma.
The proof is a routine delegation to `MonoidAlgebra.Submodule.exists_isCompl`
...
`IsSemisimpleModule` class. Technical lemma. The proof is a routine
delegation to `sSup_simples_eq_top_iff_isSemisimpleModule`. -/
...
of simple submodules. Key theorem of the section. The proof is a routine
delegation to `IsSemisimpleModule.exists_linearEquiv_dfinsupp`. -/
...
complementing any prescribed submodule `W`. Key construction of the section.
Proof idea: Zorn's lemma applied to the families `T ⊆ Set.range S` that are
sup-independent and disjoint from `W` (chains handled by
`iSupIndep_sUnion_of_directed` and directed suprema); a maximal such `T`
complements `W`, since otherwise some simple `S i` is an atom disjoint from
`W ⊔ sSup T` and adjoining it contradicts maximality. -/
...
/-- Reusable uniqueness half of GT `r10`. Technical lemma. The proof is a
routine delegation to `CompositionSeries.jordan_holder`. -/
...
standing finite-dimensional hypothesis implies finite length. Key theorem of
the section. Proof idea: finite length yields a composition series with
bottom head and top last (`isFiniteLength_iff_exists_compositionSeries`), and
any two such series are equivalent by Jordan--Hölder. -/
...
Auxiliary construction: the submodule of the product of tuples vanishing on
all coordinates `≥ k`, i.e. the partial direct sum of the first `k` factors. -/
...
/-- The initial prefix submodule is bottom. Technical lemma. The proof is
trivial: an element vanishing on all coordinates `≥ 0` is `0`. -/
...
/-- The terminal prefix submodule is everything. Technical lemma. The proof
is trivial: no coordinate of `Fin n` is `≥ n`. -/
...
Key lemma. Proof idea: project the quotient onto coordinate `k`; the
projection is surjective (witnessed by `Pi.single`) with kernel identified
with the comap of the smaller prefix, so `LinearMap.quotKerEquivRange` gives
the isomorphism. -/
...
/-- Adjacent prefix submodules differ by one simple coordinate. Technical
lemma. Proof idea: the same coordinate projection identifies the quotient by
the smaller prefix with the simple summand, and `covBy_iff_quot_is_simple`
transfers simplicity of the quotient to the covering relation. -/
...
quotient of their images. Technical lemma. Proof idea: the equivalence
restricted to `Q` (via `Submodule.submoduleMap`) carries the comap of `P`
onto the image of `P`, and `Submodule.Quotient.equiv` packages the induced
...
order.  Its factors are the displayed simple summands. Auxiliary
construction: the `i`-th term is the `i`-th prefix submodule, with covering
steps supplied by `finite_DirectSum_prefix_cov_by`. -/
...
/-- GT `r10c`, cardinality half: linearly equivalent finite direct sums of
simple modules have equally many summands. Key lemma. Proof idea: compute
composition length of the product in two ways — the length of a product is
the sum of the (unit) lengths of the simple summands
(`Module.length_pi_of_fintype`) and is preserved by the equivalence
(`LinearEquiv.length_eq`) — then compare. -/
...
only if their summands agree up to a permutation. Key theorem of the section.
Proof idea: the prefix composition series of `V` is pushed forward along `e`
to a composition series of the `W`-side product (images of coverings under an
injective map); Jordan--Hölder yields the permutation `σ` of the factors, and
each factor is identified with the corresponding summand by composing the
factor isomorphisms (`finiteDirectSumPrefix_factor_equiv`,
`Submodule.quotientEquiv_map_linearEquiv`, and Jordan--Hölder's chosen
equivalence). -/
...
sums of isotypic components. Key lemma. The proof is a routine delegation to
...
/-- GT `r9c`: submodules of semisimple modules are semisimple. Technical
lemma. The proof is trivial: it is the registered instance for submodules of
semisimple modules, found by `infer_instance`. -/
...
Technical lemma. The proof is trivial: it is the registered instance for
quotient modules of semisimple modules, found by `infer_instance`. -/
...
submodules is semisimple. Technical lemma. Proof idea: rewrite the supremum
as a supremum over the full index set and apply
...
isomorphism at the level of functions or zero. Key lemma. The proof is a
routine delegation to `LinearMap.bijective_or_eq_zero`. -/
...
disambiguates while preserving the snake_case form. Technical lemma. The proof
is a routine delegation to Mathlib's `jacobson_density`. -/
...
/-- Auxiliary construction: restriction of scalars embeds the algebra of
`R`-linear endomorphisms into the algebra of `F`-linear endomorphisms. -/
...
/-- Technical lemma: the `R`-linear endomorphisms are exactly the `F`-linear
endomorphisms commuting with the image of the `R`-action. Proof idea: an
`R`-linear endomorphism is `F`-linear and commutes with every scalar action
`Algebra.lsmul a`, giving `⊆`; conversely, commutation with all `lsmul a` is
exactly the `map_smul'` clause making an `F`-linear endomorphism `R`-linear. -/
...
/-- GT `r19`, reusable finite-family form of Jacobson density. Technical
lemma. Proof idea: apply the finset-level density theorem `jacobson_density` to
the image `Finset.univ.image v` and specialize the resulting scalar to each
`v i`. -/
...
/-- Technical lemma, the `Module.End` form of GT `r17`: the abstract
bicommutant acts through exactly the image of `A`. Proof idea: `⊆` by Jacobson
density applied to a finite `F`-basis of `V` (`jacobson_density_finite_family`),
which exhibits an endomorphism of the centralizer as scalar multiplication;
`⊇` because the `A`-action is `F`-linear and commutes with the `A`-action. -/
...
/-- GT `r17`. Main theorem of the section: if `V` is a faithful
finite-dimensional semisimple `A`-module, then the bicommutant of the image of
`A` in `End_F(V)` is that image. Proof idea: both sides are transported through
the restriction-of-scalars range identity
(`Module.End.range_restrictScalarsAlgHom_eq_centralizer`), reducing the claim
to the bicommutant computation for `Module.End A V`
(`bicommutantModuleEnd_range_eq_action_range`). -/
...
semisimple. Technical lemma. The proof is a routine delegation to
`IsSimpleRing.isSemisimpleRing_iff_isArtinianRing`, with Artinianness supplied
by `IsArtinianRing.of_finite`. -/
...
universe) have the same isomorphism type. Supporting definition: a `Prop`
packaging uniqueness of the simple-module type up to linear equivalence. -/
...
uniqueness of the simple-module type. Technical lemma. Proof idea: in one
direction, realize each simple module as a minimal ideal and chain linear
equivalences through the common simple type; conversely, apply the assumed
uniqueness to two simple submodules of the regular module. -/
...
/-- A nonzero map out of a direct sum is nonzero on one summand. Technical
lemma. Proof idea: if the map vanished after every inclusion
`DFinsupp.lsingle i`, then induction on `DFinsupp` elements (empty support,
then adding single-support summands) would force the map itself to be zero,
contradicting `hf`. -/
...
Key lemma. Proof idea: a nonzero vector yields a surjection `A →ₗ M`, since
its image is a nonzero submodule of the simple module `M`; composing with the
regular decomposition `e` and restricting to one summand gives, by simplicity,
an isomorphism onto some `S i`; uniqueness follows since isomorphic
representatives have equal indices by `hne`. -/
...
left ideals. Technical lemma. The proof is a routine delegation to
`isIsotypic_self_iff_allSimpleModulesIsomorphic`, after deriving semisimplicity
via `IsSimpleRing.isSemisimpleRing_iff_isArtinianRing`. -/
...
direct sum of copies of any chosen simple module. Technical lemma. Proof
idea: uniqueness of the simple type makes the regular module isotypic of type
`S`, and the isotypic-of-type decomposition (`htype.linearEquiv_fun`) produces
the finite multiplicity `n`. -/
...
is a direct sum of copies of any chosen simple module. Technical lemma. Proof
idea: as in the finite case, the module is isotypic of type `S`, and the
isotypic-of-type decomposition (`htype.linearEquiv_finsupp`) yields the index
type `ι`. -/
...
field dimension are isomorphic as `A`-modules. Technical lemma. Proof idea:
uniqueness of the simple type identifies `M` and `N` with finite sums of
copies of `S`; comparing `F`-dimensions forces the multiplicities to agree
(`m * dim S = n * dim S` with `dim S ≠ 0`), and the reindexing equivalence
composes the two decompositions. -/
...
composition of the `i`th projection with `e`. Technical definition. -/
...
whole product acting through the factor projection. Technical lemma. Proof
idea: the factor projection is surjective, so the identity map is a bijective
semilinear map from `S i` with its `B i`-structure to `S i` with the
pulled-back `A`-structure, and simplicity transfers along it. -/
...
isomorphic. Technical lemma. Proof idea: pick the product element that is `1`
in factor `i` and `0` in factor `j`; acting with it fixes a nonzero vector on
`S i` but kills `S j`, so an `A`-linear isomorphism would send a nonzero
vector to zero, contradicting injectivity. -/
...
Key lemma. Proof idea: transport the regular module across `e` to the product
of the factors, decompose each factor by its own finite-copy decomposition
into simples, and regroup the copies into a `DFinsupp` indexed by
`Σ i, Fin (n i)`. -/
...
Key lemma of the section. The proof assembles the equivalence from
`isSimpleRing_isArtinianRing_iff` and
`isIsotypic_self_iff_allSimpleModulesIsomorphic`, closed by `tfae_finish`. -/
...
algebra. Main theorem statement in Wedderburn--Artin form. The proof is a
routine delegation to `IsSimpleRing.exists_algEquiv_matrix_divisionRing`. -/
...
factor can also be chosen finite-dimensional over the base field. Technical
variant. The proof is a routine delegation to
`IsSimpleRing.exists_algEquiv_matrix_divisionRing_finite`, with Artinianness
supplied by `IsArtinianRing.of_finite`. -/
...
over division algebras. Main theorem statement in Wedderburn--Artin form. The
proof is a routine delegation to
...
Technical variant. The proof is a routine delegation to
...
closed field: the division factors are the base field itself. Technical
interface lemma. The proof is a routine delegation to
...
chapter's standing definition of an `F`-algebra. Key construction. The proof
is a routine delegation to `IsAlgClosed.algebraMap_bijective_of_isIntegral`
(the algebra map `F → D` is bijective), inverted via `AlgEquiv.ofBijective`. -/
...
/-- GT `r29`, part (a): for a finite product of Artinian simple rings, the
simple modules induced from chosen simple factor modules are pairwise
nonisomorphic and exhaust all simple modules. Key lemma. Proof idea: the
regular module decomposes as a direct sum of the induced modules (with
multiplicities), so the general classification lemma for simple modules of a
decomposed regular module applies, using simplicity of the induced modules and
nonisomorphism across distinct factors. -/
...
Key lemma. Proof idea: semisimplicity decomposes `M` into finitely many simple
summands; classifying each summand against the induced factor modules labels
it by some `c j`, and reassembling the summands along these labels gives the
equivalence. -/
...
is equivalent to equality of every multiplicity `rᵢ`. Key lemma. Proof idea:
forward, delegate to the finite-direct-sum cancellation lemma
`finite_directSum_simple_equiv` and rule out cross-factor isomorphisms by
distinctness of the factors; backward, reindex the coordinates along `σ`. -/
...
algebras over that field. Main theorem of the section (Maschke +
Wedderburn--Artin). Proof idea: by Maschke (`group_algebra_isSemisimple`) the
group algebra is semisimple, and `|G|` is nonzero in `F`, so the semisimple
Wedderburn--Artin decomposition over an algebraically closed field applies
with division factors equal to `F`. -/
...
elements. Technical lemma. Proof idea: write the conjugation as multiplication
by a group element `c`; centrality applied to the basis element
`MonoidAlgebra.single c 1`, compared at the coefficient `c * a`, yields the
equality. -/
...
equivalence wrapping is target-specific. Supporting definition. The proof is
trivial: it unpacks and repacks centre membership along `e` and `e.symm`. -/
...
/-- The centre of a finite product is the product of the centres. Supporting
definition. The proof delegates the centre-of-product equality to Mathlib's
`Subalgebra.center_pi` and the coordinatewise membership to
`Subalgebra.mem_pi`. -/
...
Key construction. Proof idea: read the coefficient at a representative of each
class; the inverse builds the element whose coefficient at `g` is the value on
the class of `g`, whose centrality follows by induction on group-algebra
elements together with conjugate-invariance of coefficients; the left inverse
uses `MonoidAlgebra.coeff_eq_of_mem_center`. -/
...
central element corresponding to the delta function at `C`. Supporting
definition. The proof is a routine delegation to
`MonoidAlgebra.centerEquivClassFunction.symm` applied to the `Pi.single` at
`C`. -/
...
Technical lemma. The proof is trivial: it unwraps `MonoidAlgebra.conjClassSum`
and `MonoidAlgebra.centerEquivClassFunction` and simplifies `Pi.single_apply`. -/
...
number of conjugacy classes. Technical lemma. The proof is a routine
delegation to `MonoidAlgebra.centerEquivClassFunction`: transport the
dimension along that linear equivalence and compute the dimension of functions
on a fintype. -/
...
is simple. Technical lemma. Proof idea: given a nonzero vector `v` (coordinate
`j` nonzero), the matrix whose `j`-th column is `w i / v j` and whose other
columns vanish maps `v` to `w`; hence every nonzero vector generates the
module (`isSimpleModule_iff_toSpanSingleton_surjective`). -/
...
field has dimension equal to the number of factors. Technical lemma. Proof
idea: each full matrix algebra has centre equal to its scalar subalgebra
(`Algebra.IsCentral.center_eq_bot`), so the product centre identifies, via
`Subalgebra.centerPiLinearEquiv`, with `ι → k`, whose dimension is `card ι`. -/
...
/-- GT `r32`, part (a), factor-count form: every matrix-product presentation
of a finite group algebra has one factor per conjugacy class. Key lemma.
Proof idea: compute `n` as the dimension of the product centre (each factor
contributes `1`), transport the dimension to the group-algebra centre along
the algebra equivalence, and compare with the number of conjugacy classes. -/
...
the factor index has the cardinality of the conjugacy-class set. Key lemma.
Proof idea: transport the classification of simple modules of a finite product
of matrix algebras (`RingEquiv.pi_factor_simple_Module_classification`, each
factor's standard column module being simple by `Matrix.isSimpleModule_pi`)
across the presentation `e`; the factor count is the number of conjugacy
classes (`MonoidAlgebra.card_matrix_factors_eq_card_conj_classes`). -/
...
/-- GT `r32(a)`, source-facing form. Main theorem of the section: over an
algebraically closed characteristic-zero field, the group algebra has a
matrix-factor presentation; those factors uniquely enumerate all simple
modules, and their number is the number of conjugacy classes. Proof idea: take
the Wedderburn--Artin presentation (`group_algebra_exists_algEquiv_pi_matrix`);
the factor count is the number of conjugacy classes
(`MonoidAlgebra.card_matrix_factors_eq_card_conj_classes`), and uniqueness of
the simple module per factor is the matrix-factor classification
(`MonoidAlgebra.matrix_factor_simple_Module_classification`). -/
...
column modules. Supporting definition: the transposition-style map
`X ↦ its columns`, linear over the matrix algebra, with compatibility of the
`A`-action checked via `Matrix.mul_apply`. -/
...
`i`th factor. Key lemma. Proof idea: transport the regular module across `e`
to the product of matrix algebras, split each matrix factor into its column
modules, and regroup the copies into a `DFinsupp` indexed by `Σ i, Fin (d i)`. -/
...
Key lemma. The dimension clause is a routine computation of the finrank of
`Fin (d i) → k`; the decomposition clause is a routine delegation to
`RingEquiv.regular_linearEquiv_pi_factor_columns`. -/
...
/-- Dimension form of GT `r32(c)` for the matrix degrees: the squares of the
matrix sizes sum to the group order. Technical lemma. Proof idea: compare
dimensions on both sides of the algebra equivalence; the product side has
dimension `∑ i, d i ^ 2`, and the group algebra has dimension `card H` via its
...
simple module. Technical restatement. The proof is a routine delegation to
`MonoidAlgebra.sum_sq_matrixFactors_eq_card` after simplifying the finrank of
`Fin (d i) → k` to `d i`. -/
...
squares of the dimensions sum to the group order. Main theorem of the section.
Proof idea: take the Wedderburn--Artin matrix presentation of the group
algebra and repackage, clause by clause, the classification of simples for
matrix products, the regular-module decomposition, and the dimension count. -/
...
is the sum of the two characters. Technical lemma. The proof is a routine
delegation to `LinearMap.trace_prodMap'`. -/
...
subspace. Technical lemma. The proof is a routine delegation to
...
space of equivariant maps. Technical lemma. The proof is a routine delegation
to `FDRep.scalar_product_char_eq_finrank_equivariant`. -/
...
/-- Class functions, represented without choosing representatives of
conjugacy classes: functions on `ConjClasses G`. Supporting definition. -/
abbrev ClassFunction := ConjClasses G → k

/-- An `F`-valued Hermitian inner product for a conjugation-stable subfield
of `ℂ`. Positivity is stated after the specified embedding. Supporting
definition: a bundled structure recording the axioms of a positive Hermitian
form read through `ι`. -/
...
/-- Average of the product of two class functions over a finite group,
conjugated in the second factor. Supporting definition: the source pairing,
formed as `⅟(card H)` times the sum of `f₁(a) * star (f₂(a))` over group
elements. -/
...
Key construction. Proof idea: each axiom is verified by unfolding the finite
average (`ring` for the bilinear laws, `hstar` for conjugate symmetry), and
positivity identifies the embedded pairing with `⅟(card H)` times a sum of
complex norm squares, positive because `f` is nonzero at some class and `ι` is
injective. -/
...
/-- A representation character factored through conjugacy classes. Key
construction. Proof idea: descend the character `V.character` along the
conjugacy quotient; well-definedness is the conjugation-invariance of
characters (`FDRep.char_conj`). -/
...
the averaged scalar product with `V.character`. Supporting definition. The
proof is trivial: additivity and homogeneity of the finite average, checked by
`ring`. -/
...
open scoped Classical in
/-- GT `r33`, also the linear-independence half of `r39`: characters of
pairwise nonisomorphic simple representations are linearly independent as
class functions. Key lemma. Proof idea: apply the character pairing against
`V i` to a null combination; orthonormality of simple characters turns the
combination into the Kronecker delta at `i`, forcing every coefficient to
vanish. -/
...
/-- Character equality induced by an isomorphism, after passage to class
functions. Technical lemma. Proof idea: isomorphic representations have equal
characters pointwise (`FDRep.char_iso`); the equality transports along
induction on conjugacy classes. -/
...
the ground field descends to genuine `ℤ`-linear independence. Technical lemma.
Proof idea: cast an integral null combination to `k` and apply the `k`-linear
independence of simple characters; each coefficient vanishes in `k`, and
injectivity of the `ℤ → k` cast (CharZero) forces it to vanish in `ℤ`. -/
...
characters gives a permutation matching their simple constituents. Key lemma.
Proof idea: pair both sides against `V i`; orthonormality reduces each side to
the multiplicity of `V i`, recorded as a cardinality of fibres of `c` and `d`,
so the fibres have equal sizes and a fibre-wise equivalence assembles into the
permutation. -/
...
the finite direct-sum/Jordan--Hölder reconstruction bridge. Key lemma. Proof
idea: forward, an isomorphism equalizes characters; backward, equal characters
of the two sums yield a permutation matching the constituents via
`FDRep.exists_equiv_of_sum_simple_character_eq`, to which `hreconstruct` is
applied. -/
...
/-- GT `ga13m`. Main theorem of this section: every finite group of order `2 * p`, for `p` an
odd prime, is cyclic or isomorphic to the dihedral group of order `2 * p`.

Mathlib's `DihedralGroup p` has cardinality `2 * p`; its elements are
`r a` and `sr a` for `a : ZMod p`.

Proof idea: pick elements `r`, `s` of orders `p` and `2` (via
`exists_prime_orderOf_dvd_card`). Conjugation `s * r * s⁻¹ = r ^ i` forces
`i * i ≡ 1 [ZMOD p]`, hence `s * r * s⁻¹ = r` or `= r⁻¹`. In the commuting case the coprime
orders make `r * s` an element of order `2 * p`, so the group is cyclic; in the inverting
case the closure of `{r, s}` is everything, and
`nonempty_mulEquiv_dihedral_of_generators` supplies the isomorphism. -/
...
/-- Supporting definition: a group is complete when its center is trivial and every
automorphism is inner (`∀ α : MulAut A, ∃ a : A, α = MulAut.conj a`). -/
def IsCompleteGroup (A : Type*) [Group A] : Prop :=
  Subgroup.center A = ⊥ ∧
    ∀ α : MulAut A, ∃ a : A, α = MulAut.conj a

/-- Technical lemma: completeness is invariant under group equivalence.
Proof idea: the center transports along `e` by injectivity, and an automorphism `β` of `B`
is conjugated back through `e` to the automorphism `e.trans (β.trans e.symm)` of `A`; if
that is conjugation by `a`, then conjugation by `e a` realizes `β`. -/
...
/-- Supporting definition: an exact group extension `1 → N → G → Q → 1`, packaged as an
injective inclusion and a surjective projection whose kernel is the image of the inclusion,
without identifying `N` with its image. -/
...
/-- Key lemma: splitting of an exact extension (existence of a homomorphic section) is
equivalent to the existence of a subgroup whose composition with the projection is
bijective.
Proof idea: the range of a section is such a subgroup — injectivity holds because the
section value is determined by the projection, surjectivity by the section itself;
conversely, bijectivity makes the projection restrict to a multiplication isomorphism
`H ≃* Q` (via `MulEquiv.ofBijective`), and composing its inverse with the inclusion gives
the section. -/
...
/-- Key lemma: for an exact extension with complete kernel, the image of the kernel and its
centralizer are complementary subgroups and commute elementwise.
Proof idea: exactness makes the kernel image normal and identifies it with the complete
group `N` (completeness transfers via `IsCompleteGroup.mulEquiv`); completeness makes every
conjugation on the image inner, which factors each `g` as an image element times an element
centralizing the image; the intersection is trivial because an element of the image that
centralizes it lies in its center, which is trivial. -/
...
/-- Main theorem of this section: an exact extension with complete kernel splits, and the
kernel image together with its centralizer forms an internal direct product.
The proof is a routine delegation to `sectionOfComplete` (with
`projection_sectionOfComplete`) and to `directProductEquivOfComplete` (with its evaluation
lemma `directProductEquivOfComplete_apply`). -/
...
/-- GT `r28`. Key lemma: the centralizer of a semisimple finite-dimensional action is a
finite product of simple matrix algebras over division algebras, hence semisimple.
Proof idea: identify the centralizer with the range of the scalar-restriction algebra
homomorphism, transport semisimplicity of `Module.End A V` along the resulting algebra
equivalence (`eC.isSemisimpleRing`), and apply
`IsSemisimpleRing.exists_algEquiv_pi_matrix_divisionRing`. -/
...
by its half-power exactly when its exponent is divisible by `n`. Technical
lemma. Proof idea: membership gives `x ^ m = (x ^ n) ^ k`, i.e.
`x ^ (n * k) = x ^ m`; since `orderOf x = 2 * n`, `x ^ a = x ^ b` is
equivalent to `2 * n ∣ a - b` (`orderOf_dvd_sub_iff_zpow_eq_zpow`), and
`2 * n ∣ n * k - m` forces `n ∣ m`; conversely `x ^ (n * k) = (x ^ n) ^ k`
lies in the zpowers of `x ^ n`. -/
...
half-power divides its exact order by two. Technical lemma. Proof idea:
`x ^ n = z` lies in the quotient kernel, so the order divides `n`; conversely,
if `x ^ m` is trivial in the quotient then `x ^ m ∈ ⟨z⟩`, and
`mem_zpowers_pow_dvd` gives `n ∣ m`, so minimality (`orderOf_eq_iff`) forces
the order to be exactly `n`. -/
...
/-- Supporting definition: the algebra homomorphism sending an opposite scalar `d` to
componentwise right multiplication by `d` on the standard column module — right scalars map
into the commutant of the left matrix action. -/
...
Technical lemma. Proof idea: as for `Matrix.isSimpleModule_pi`: given a
nonzero vector `x` (coordinate `j` nonzero), the matrix whose `j`-th column is
`y i / x j` and whose other columns vanish maps `x` to `y`, so every nonzero
vector generates the module. -/
...
/-- Technical lemma: if a finite-dimensional algebra over `F` is presented as a nonempty
full matrix algebra over a division ring `D`, then `D` is finite-dimensional over `F`.
Proof idea: transport finite-dimensionality through the presentation and embed `D` into the
matrix algebra by the single-entry map `d ↦ Matrix.single i₀ i₀ d`, which is injective;
then apply `FiniteDimensional.of_injective`. -/
...
/-- Technical lemma: once the division algebras in two nonempty matrix presentations are
identified over `F`, comparison of `F`-dimensions identifies the sizes.
Proof idea: `finrank F A = n^2 * finrank F D = m^2 * finrank F E` and the given equivalence
gives `finrank F D = finrank F E`; since `finrank F D > 0`, cancelling yields `n * n = m * m`
and hence `n = m`. -/
...
/-- GT `r23`. Main theorem of this section: both the matrix size and the coefficient
division algebra in a matrix presentation of a finite-dimensional simple algebra are unique.
Proof idea: the standard column modules become simple `A`-modules (simplicity transported
from `Matrix.isSimpleModule_column` through the presentations); Schur's lemma over the
algebraically closed ground field identifies the `A`-endomorphism algebras, and
`matrixEndTransportAlgEquiv` together with `Matrix.columnModuleEndAlgEquiv` yields
`Dᵐᵒᵖ ≃ₐ[F] Eᵐᵒᵖ`, whose unopposite gives the coefficient equivalence; sizes compare by
`matrix_presentation_unique_sizes`. -/
...
/-- Technical lemma: the trace of a pointwise endomorphism of a finite dependent product is
the sum of the traces of its components.
Proof idea: induction on `n`, splitting off the first coordinate with the explicit
`Fin.cons` equivalence; conjugation-invariance of the trace (`LinearMap.trace_conj'`)
reduces to `LinearMap.trace_prodMap'`, then the induction hypothesis and
`Fin.sum_univ_succ`. -/
...
/-- Technical lemma: character additivity for the finite direct sum constructed through the
modules of the group algebra.
Proof idea: unfold `FDRep.dfinsuppOf` and use `ofModule'_trace_add` to move to the
group-algebra-module picture; each summand trace is conjugation-invariant under
...
/-- Technical lemma: the character of a finite direct sum is the sum of the characters of
its summands, as class functions.
Proof idea: evaluate on representatives (`Quotient.inductionOn`) and apply
`Finset.sum_apply` to reduce to `FDRep.character_dfinsupp_of_apply`. -/
...
/-- Technical lemma: a group-algebra-linear equivalence from the module of a representation
`ρ` to `M` induces an intertwining equivalence between `ρ` and the representation
constructed from `M`.
Proof idea: compose `ρ.asModuleEquiv.symm` with the scalar restriction of `e` to obtain a
`k`-linear isomorphism `V ≃ₗ[k] M`; it intertwines the actions because `e` is
`MonoidAlgebra`-linear (`e.map_smul`). -/
...
/-- Key lemma: a simple group-algebra module yields a categorically simple finite
representation after applying `ofModule'`.
Proof idea: use `FDRep.simple_iff_end_is_rank_one`; transport endomorphisms of the
representation to endomorphisms of the group-algebra module (via the forgetful and
intertwining-map equivalences), where Schur's lemma over an algebraically closed field
(`IsSimpleModule.algebraMap_end_bijective_of_isAlgClosed`) makes the endomorphism algebra
one-dimensional. -/
...
/-- Key construction: Maschke decomposition, reconstructed as an isomorphism to an actual
finite DFinsupp representation from a complete family of simple modules.
Proof idea: semisimplicity of the group algebra
(`IsSemisimpleModule.exists_linearEquiv_fin_dfinsupp`) decomposes the associated module
into finitely many simple modules; the completeness hypothesis identifies each with some
`V i`, and `Representation.nonempty_equiv_of_asModule_linearEquiv_ofModule'` lifts the
module equivalence to a representation isomorphism. -/
...
/-- Auxiliary construction: reindex a finite dependent direct sum by an index equivalence
while changing each summand by a linear equivalence.
The construction is a routine composition of `DFinsupp.domLCongr` and
...
/-- Key lemma: a group-algebra-linear equivalence of finite-dimensional modules induces an
isomorphism of the corresponding finite representations constructed by `ofModule'`.
Proof idea: the underlying `k`-linear isomorphism of `e` intertwines the actions because
`e` is `MonoidAlgebra`-linear; package it as a `Representation.Equiv` and lift it to an
`FDRep` isomorphism with `Action.mkIso`, checking equivariance pointwise. -/
...
/-- GT `r34`. Main theorem of this section: finite-dimensional representations over an
algebraically closed characteristic-zero field are isomorphic exactly when their characters
agree.
Proof idea: fix the complete simple family; both representations decompose into it, and
character iso-invariance plus additivity (`FDRep.characterClassFunction_dfinsupp_of`)
express their characters as sums of simple characters. Equality of characters forces equal
multiplicities (`FDRep.nonempty_iso_iff_character_eq_of_simple_decompositions`), and the
isomorphism is reassembled through `dfinsuppReindexLinearEquiv` and
`fd_rep_ofModule'_iso_of_linearEquiv`. -/
...
/-- GT `r34a`. Main theorem of this section: the simple characters form an integral basis of
the virtual characters.
Proof idea: take the complete simple family; every representation decomposes into it, so the
simple characters span the virtual characters (packaged by
`FDRep.simpleVirtualCharacterBasis`), and pairwise non-isomorphism gives the basis property
(`FDRep.simpleVirtualCharacterBasis_apply`). -/
...
variable {B : Type*}
/-- Technical instance: decidability of equality on `B`, supplied by classical choice.
The proof is a routine delegation to `Classical.decEq`. -/
local instance decEqB1 : DecidableEq B := Classical.decEq B

/-- Supporting definition: the Coxeter coefficient between two roots, `-1` when `M s t = 0`
and otherwise `-cos (π / M s t)`; it is the off-diagonal entry of the pair form. -/
noncomputable def pairCoeff (M : CoxeterMatrix B) (s t : B) : ℝ :=
  if M s t = 0 then -1 else -Real.cos (Real.pi / (M s t : ℝ))

/-- Supporting definition: the Coxeter quadratic form
`a^2 + b^2 + 2 * pairCoeff M s t * a * b` on the two-dimensional span of two roots. -/
...
/-- GT `fg17`. Key lemma: the Coxeter form on the span of two distinct roots is positive
semidefinite for an infinite entry (`M s t = 0`) and positive definite for a finite entry.
Proof idea: in the infinite case the coefficient is `-1`, so the form is `(a - b)^2`; in the
finite case complete the square,
`pairForm = (b + pairCoeff M s t * a)^2 + (1 - pairCoeff M s t ^ 2) * a^2`, using
`pairCoeff_bounds` (`|pairCoeff M s t| < 1`). -/
...
/-- Supporting definition: the Coxeter quadratic form on the real vector space `Space B`,
defined by summing `a * b * coeff M i j` over the supports of `x` and `y`. -/
...
/-- Supporting definition: the reflection across the hyperplane orthogonal to the root of
`s`, `x ↦ x - (2 * form M x (root s)) • root s`, as a linear map. -/
...
/-- Supporting definition: the reflection across `root s` as a linear equivalence, obtained
from the linear map `reflection` by `LinearEquiv.ofInvolutive` using
...
/-- Key lemma: decompose an arbitrary vector into a component in the span of two distinct,
non-orthogonal roots plus a component orthogonal to both roots.
Proof idea: the Gram matrix `[[1, c], [c, 1]]` of the two roots, with `c = pairCoeff M s t`
of absolute value `< 1` (`pairCoeff_bounds`), is invertible, so the projection coefficients
`a`, `b` are solved in closed form; the remainder `h` then pairs to zero with both roots by
direct computation. -/
...
/-- Supporting definition: the embedding of the two-dimensional pair plane `(Fin 2 → ℝ)`
into `Space B`, `(a, b) ↦ a • root s + b • root t`. -/
...
/-- Supporting definition: the product of the two rank-one pair reflections,
...
/-- Supporting definition: the linear action of the pair reflection product on the
coordinate plane, the explicit map
`z ↦ ![(4 * c ^ 2 - 1) * z 0 + 2 * c * z 1, -2 * c * z 0 - z 1]` with
`c = pairCoeff M s t`. -/
...
/-- Supporting definition: the action of the pair reflection product on the coordinate plane
as a linear equivalence, bundling `pairActionLinear` with its bijectivity
(`pairActionLinear_bijective`) via `LinearEquiv.ofBijective`. -/
...
/-- Supporting definition: the complex coordinates of a vector in the pair plane,
`z ↦ z 0 + (c + d * i) * z 1`, as a linear map `(Fin 2 → ℝ) →ₗ[ℝ] ℂ`. -/
...
/-- Technical lemma: the exact order of the pair action on the coordinate plane is `M s t`,
with `0` meaning infinite order (Mathlib's convention for `orderOf`).
The proof is a routine case split delegating to `pairAction_order_finite` and
`pairAction_order_infty`. -/
...
/-- Technical lemma: if `P` intertwines through an injective linear map `E` down to `A`
(`P ∘ E = E ∘ A`), then the exact order of `A` divides the exact order of `P`.
Proof idea: induction shows `E ((A ^ n) z) = (P ^ n) (E z)`; if `A ^ m = 1` then `P ^ m`
fixes the range of the injective `E` pointwise, hence `P ^ m = 1`. -/
...
/-- Key lemma: any model of the Coxeter relations with the prescribed exact orders — an
injective assignment `R : B → H` with each `R s` of order `2` and each `R s * R t` of exact
order `M s t` — proves faithfulness of `M.simple` and the same exact orders in the
presented Coxeter group `M.Group`.
Proof idea: the universal property gives a lift `φ : M.Group →* H` with
`φ (M.simple s) = R s`; the defining relations give `orderOf (M.simple s) ∣ 2` and
`orderOf (M.simple s * M.simple t) ∣ M s t` (`CoxeterSystem.simple_sq`,
`CoxeterSystem.simple_mul_simple_pow`), while `φ` pulls back the model's exact orders, so
`Nat.dvd_antisymm` gives equality; injectivity of `M.simple` follows because
`φ ∘ M.simple = R` is injective. -/
...
/-- Key lemma: the pair reflection product satisfies the Coxeter relation
`(pairProduct M s t) ^ (M s t) = 1` on the whole `Space B`, not just the two-dimensional
span of the two roots.
Proof idea: `decompose_into_span_orthogonal` reduces a general vector to a span component —
handled by `pairEmbed_intertwines` and the exact order of `pairAction` (`pairAction_order`)
— plus an orthogonal component fixed pointwise by both reflections
...
/-- Key lemma: the pair reflection product has exact order `M s t` on `Space B`.
Proof idea: `pairProduct_pow_orderOf_eq_one` gives
`(pairProduct M s t) ^ (M s t) = 1`; the reverse divisibility follows from
`LinearEquiv.orderOf_dvd_of_intertwines`, since `pairProduct` intertwines through the
injective embedding `pairEmbed` down to `pairAction`, whose exact order is `M s t`
(`pairAction_order`). -/
...
/-- GT `fg16`. Main theorem of the Coxeter section: the geometric (Tits) representation of
the presented Coxeter group is faithful, and the simple reflections and their pairwise
products have the prescribed exact orders.
The proof is a routine delegation to `exact_orders_of_model` applied to the reflection
model `fun s => reflectionEquiv M s`, with the Coxeter relations and exact orders verified
via `reflection_involutive`, `pairProduct_pow_orderOf_eq_one`, `reflectionEquiv_orderOf`,
and `pairProduct_orderOf_eq`. -/
...
/-- Supporting definition: the two-by-two diagonal block `Matrix.diagonal ![z, z⁻¹]` of a
Bd3m source. -/
...
/-- Supporting definition: packaging a determinant-one matrix as an element of
`Matrix.SpecialLinearGroup (Fin 2) K`, i.e. the special-linear component of a Bd3m source. -/
...
/-- Supporting definition: the upper-triangular block `!![z, 1; 0, z⁻¹]` of a Bd3m source. -/
def upper (z : Kˣ) : Matrix (Fin 2) (Fin 2) K :=
  !![(z : K), 1; 0, (z⁻¹ : K)]

/-- Supporting definition: the lower-triangular block `!![z, 0; t, z⁻¹]` of a Bd3m source. -/
def lower (z : Kˣ) (t : K) : Matrix (Fin 2) (Fin 2) K :=
  !![(z : K), 0; t, (z⁻¹ : K)]

/-- Supporting definition: the upper block `upper z` viewed in `SL₂`; its determinant is
one by construction. -/
def upperSL0 (z : Kˣ) : Matrix.SpecialLinearGroup (Fin 2) K :=
  slOf (upper z) (by simp [upper, Matrix.det_fin_two])

/-- Supporting definition: the lower block `lower z t` viewed in `SL₂`; its determinant is
one by construction. -/
def lowerSL0 (z : Kˣ) (t : K) :
    Matrix.SpecialLinearGroup (Fin 2) K :=
  slOf (lower z t) (by simp [lower, Matrix.det_fin_two])

/-- Supporting definition: the product of the `SL₂` blocks of a Bd3m source,
...
/-- GT `bd3m`. Main theorem of this section: element orders impose no restriction on the
order of their product in a suitable finite group — for any `m, n, r > 1` there is a finite
group with elements `a`, `b` of orders `m`, `n` whose product `a * b` has order `r`.
The proof is a routine delegation to `Bd3mSource.bd3m_source_clean`. -/
theorem bd3m
    {m n r : ℕ} (hm : 1 < m) (hn : 1 < n) (hr : 1 < r) :
    ∃ (G : Type) (_ : Group G) (_ : Finite G), ∃ a b : G,
      orderOf a = m ∧ orderOf b = n ∧ orderOf (a*b) = r := by
...
Technical lemma (monotonicity certificate): `Monotone (fun j => f (sortedEquiv f j))`, i.e.
the enumeration `sortedEquiv f` lists `α` in non-decreasing order of `f`.
The proof is trivial: it is the second component of `sortedEquivData f`. -/
...
/-- GT `it21(a,b)`. Main theorem of this section: source-facing invariant-factor
decomposition — the number of infinite cyclic factors is the intrinsic free rank
`CommGroup.freeRank G`, while the finite cyclic factors are nontrivial and divisibility
ordered.
Proof idea: start from the abstract free-product decomposition
(`CommGroup.exists_mulEquiv_free_prod_invariantFactors`); the torsion product is finite,
hence torsion, so `CommGroup.freeRank_eq_of_free_prod_torsion` identifies the abstract free
rank with `CommGroup.freeRank G`, which is then substituted into the isomorphism. -/
...
/-- Supporting definition: `H` is a maximal proper normal subgroup of `K`, expressed by
normality of `H` in `K` and simplicity of the relative quotient `K ⧸ H`. -/
...
/-- Supporting definition: the factors represented by two adjacent subgroup pairs
`(p.1, p.2)` and `(q.1, q.2)` have isomorphic relative quotients (given normality of the
respective subgroup inclusions). -/
...
/-- Supporting definition (technical): congruence of relative quotients when the upper
subgroup is fixed, `A = B`; the construction is a routine delegation to `MulEquiv.refl`. -/
...
/-- Supporting definition (technical): congruence of relative quotients when the lower
subgroup is fixed, `N = M`; the construction is a routine delegation to `MulEquiv.refl`. -/
...
/-- Supporting definition: a group composition series, i.e. a composition series in the
Jordan–Hölder lattice `jordanHolderLattice G` of subgroups, indexed from its least subgroup
to its greatest. -/
abbrev Series (G : Type*) [Group G] :=
  @CompositionSeries (Subgroup G) inferInstance
    (jordanHolderLattice G)

/-- GT `ns02` (Jordan–Hölder). Main theorem of this section: two composition series with
endpoints `⊥, ⊤` have equal length and isomorphic quotient factors up to a permutation.
The proof is a routine delegation to `CompositionSeries.jordan_holder` in the lattice
`jordanHolderLattice G`, unpacked into a length equality and a permutation of factors. -/
...
/-- Supporting definition: a subgroup preserved by all operators, `x ∈ H → a • x ∈ H` for
the multiplicative distributive action of `A`. -/
...
/-- Key lemma: invariance of `N` supplies the `MulAction.QuotientAction A N` condition.
Proof idea: because the action is by group automorphisms (`map_inv`, `map_mul`),
`(a • x)⁻¹ * (a • y) = a • (x⁻¹ * y)`, so the quotient-action condition is exactly
invariance of `N` applied to `x⁻¹ * y ∈ N`. -/
...
/-- Technical lemma: the kernel of an equivariant group homomorphism is invariant.
The proof is trivial: equivariance `f (a • x) = a • f x` together with `f x = 1` gives
`f (a • x) = 1` (then `map_one`). -/
theorem ker (f : G →*[A] G') :
...
/-- Technical lemma: the range of an equivariant group homomorphism is invariant.
The proof is trivial: `a • f x = f (a • x)` is again in the range, by equivariance. -/
...
/-- Technical lemma: inverse images of invariant subgroups are invariant.
The proof is trivial: `f (a • x) = a • f x ∈ K`, by invariance of `K`. -/
...
/-- Technical lemma: direct images of invariant subgroups are invariant.
The proof is trivial: `a • f x = f (a • x)` with `a • x ∈ H`, by invariance of `H`. -/
...
/-- Technical lemma: an invariant subgroup cuts out an invariant subgroup of every
invariant subgroup (for the restricted action).
The proof is trivial: the restricted action acts by ambient application, so `a • x ∈ N`
follows directly from invariance of `N`. -/
...
/-- Supporting definition: a multiplicative equivalence commuting with the operator action,
i.e. a `G ≃* G'` together with the equivariance condition
`toMulEquiv (a • x) = a • toMulEquiv x`. -/
...
/-- GT `ns24`. Main theorem of this section: the equivariant first isomorphism theorem.
Proof idea: the ordinary kernel-quotient/range isomorphism is equivariant because kernel
and range are invariant (`IsInvariant.ker`, `IsInvariant.range`); every conjunct then
delegates to `MonoidHom.normal_ker`, `QuotientGroup.mk'_surjective`, the bijectivity of
`quotientKerEquivRange`, `Subgroup.subtype_injective`, and a definitional factorization. -/
...
/-- GT `ns25`. Main theorem of this section: the equivariant second isomorphism theorem.
Proof idea: the canonical equivalence is `QuotientGroup.quotientInfEquivProdNormalQuotient`,
which is equivariant because `N.subgroupOf H` and `N.subgroupOf (H ⊔ N)` are invariant; the
remaining conjuncts — normality, the image formula
`(N.subgroupOf H).map H.subtype = H ⊓ N`, the product formula for `H ⊔ N`, and the pointwise
description of the equivalence — are pointwise computations. -/
...
/-- GT `ns26`. Key lemma: a surjective equivariant homomorphism carries the ordinary
subgroup correspondence to the invariant-subgroup correspondence.
Proof idea: the comap–map identities delegate to `Subgroup.comap_map_eq_self` and
`Subgroup.map_comap_eq_self_of_surjective`; the invariance directions delegate to
`IsInvariant.map` and `IsInvariant.comap` combined with those identities. -/
